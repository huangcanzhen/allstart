...
class Mumuplay:
            ...
            while True:
                ...
                if traget_value is None:
                    ...
                    if attempts == 5:
                        ...
                        if bmp_path == r'.\pic\WC3V3\2.bmp':
                            print('退出重来')
                            self.sendcommand('MuMuManager.exe control -v all shutdown')
                            time.sleep(1)
                            self.sendcommand('python G_run.py')
                            self.sendcommand('taskkill /im python.exe')
                            time.sleep(1)
    ...
    def click_location(self, location, bmp_path):
        if bmp_path == r'.\pic\Card\3.bmp' or bmp_path ==r'.\pic\WC3V3\2.bmp':
            self.slide(location,bmp_path)
        else:
            try:
                pyautogui.moveTo(x=location[0] + self.target_image.shape[1] // 2, y=location[1] + self.target_image.shape[0] // 2)
                pyautogui.click()
    ...
    def slide(self,location,bmp_path):
            try:
                ...
                if bmp_path == r'.\pic\WC3V3\2.bmp':
                    ...
                    # pyautogui.moveRel(0, -400, duration=1)  #  向上滑动400像素，持续1秒
